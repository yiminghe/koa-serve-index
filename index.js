 ...
 * Copyright(c) 2014 Douglas Christopher Wilson yiminghe@gmail.com
 * MIT Licensed
 */
...
exports = module.exports = function serveIndex(root, options) {
    ...
    return function *serveIndex(next) {
        ...
        if (~path.indexOf('\0')) return createError(this, 400);

        // malicious path
        if (path.substr(0, root.length) !== root) {
            debug('malicious path "%s"', path);
            return createError(this, 403);
        }
...
function createError(context, code, msg) {
    msg = msg || http.STATUS_CODES[code];
...
function stat(dir, files, cb) {
    ...
    files.forEach(function (file) {
        batch.push(function (done) {
            var stat = fs.statSync(join(dir, file));
            done(null, stat || null);
        });
    });
